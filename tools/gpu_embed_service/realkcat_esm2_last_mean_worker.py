#!/usr/bin/env python3
"""Batched RealKcat ESM2 last-layer mean embedding staging worker.

Writes persistent RealKcat cache files:
  media/sequence_info/esm2_layer_last_mean/{seq_id}.npy

Each file contains a CPU float32 vector of shape [1280], representing the
mean of residue token embeddings from esm2_t33_650M_UR50D layer 33.
"""
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batched RealKcat ESM2 last-layer mean embedding worker."
    )
...
def main() -> int:
    ...
    if not missing_ids:
        print(f"All {len(ready_ids)} RealKcat mean embeddings already staged.")
        return 0
    ...
    print(f"RealKcat ESM2 mean: computing {len(missing_ids)} missing embedding(s) on {device}.")
    ...
    def run_batch(batch_ids: list[str], committer: SpoolAsyncCommitter) -> None:
        ...
        for row_idx, seq_id in enumerate(batch_ids):
            ...
            if valid_len <= 0:
                raise RuntimeError(f"No ESM2 residues available for seq_id={seq_id}")
            mean_vec = (
                token_repr[row_idx, 1 : 1 + valid_len]
                .mean(dim=0)
                .numpy()
    ...
    print(f"RealKcat ESM2 mean: committed {len(missing_ids)} embedding(s) to {cache_dir}.")
    return 0
